...
describe 'Test hashing requirements' do
  ...
  describe 'Test cryptographic hashing' do
    describe 'Check hashes are consistently produced' do
      it 'should be the same and not nil' do
        ...
        db.each do |cp|
          ...
          cp.first.hash_secure.must_equal cp.last.hash_secure
        end
      end
      # TODO: Check that each card produces the same hash if hashed repeatedly
    end
    ...
    describe 'Check for unique hashes' do
      it 'should be different for different cards' do
        cards.combination(2).to_a.each do |cmp|
          cmp.first.hash_secure.wont_be_nil
          cmp.last.hash_secure.wont_be_nil
          cmp.first.hash_secure.wont_equal cmp.last.hash_secure
        end
      end
      # TODO: Check that each card produces a different hash than other cards
    end
    ...
    describe 'Check regular hash not same as cryptographic hash' do
      it 'hash should not be the same as hash_secure' do
        cards.each do |card|
